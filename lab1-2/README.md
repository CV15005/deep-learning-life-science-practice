...
[Блокнот для отладки](https://colab.research.google.com/drive/14eustLc7SVBtMesemDSqQoNeP216GmiK?usp=sharing) – вспомогательный jupyter-блокнот для отладки кода (для быстроты обучения используется MNIST и упрощенная модель). Блокнот позволит верифицировать, корректен ли код функций потерь и код обучения (выполнение засчитывается как лабораторная №1).
